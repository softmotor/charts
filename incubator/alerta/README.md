...
- Be careful when using functions that generate random data (like `alerta.fullname.unique`).
...
We define some of the most alerta kinds in a way that lets you easily work with
them.
...
```yaml
{{- template "alerta.service" (list . "mychart.service") -}}
...
Note that the `alerta.service` template defines two parameters:
...
The `alerta.service` template is responsible for rendering the templates with
...
The `alerta.service` template creates a basic `Service` resource with the
following defaults:
...
- Selector set to `app: {{ template "alerta.name" }}, release: {{ .Release.Name | quote }}` to match the default used in the `Deployment` resource

Example template:

```yaml
{{- template "alerta.service" (list . "mychart.mail.service") -}}
{{- define "mychart.mail.service" -}}
metadata:
  name: {{ template "alerta.fullname" . }}-mail # overrides the default name to add a suffix
...
{{- end -}}
---
{{ template "alerta.service" (list . "mychart.web.service") -}}
{{- define "mychart.web.service" -}}
metadata:
  name: {{ template "alerta.fullname" . }}-www  # overrides the default name to add a suffix
  labels:                                       # appended to the labels section
    protocol: www
spec:
  ports:                                        # composes the `ports` section of the service definition.
...
The `alerta.deployment` template defines a basic `Deployment`. Underneath the
hood, it uses `alerta.container` (see next section).

By default, the pod template within the deployment defines the labels `app: {{ template "alerta.name" . }}`
...
```yaml
{{- template "alerta.deployment" (list . "mychart.deployment") -}}
...
The `alerta.container` template creates a basic `Container` spec to be used
...
spec:
  template:
    spec:
      containers:
      - {{ template "alerta.container" (list . "mychart.deployment.container") }}
{{- end -}}
...
`alerta.container` template to populate the PodSpec's container list. The usage
...
The `alerta.configmap` template creates an empty `ConfigMap` resource that you
can override with your configuration.

Example use:

```yaml
{{- template "alerta.configmap" (list . "mychart.configmap") -}}
...
The `alerta.secret` template creates an empty `Secret` resource that you
can override with your secrets.

Example use:

```yaml
{{- template "alerta.secret" (list . "mychart.secret") -}}
...
The `alerta.ingress` template is designed to give you a well-defined `Ingress`
...
```yaml
{{- template "alerta.ingress" (list . "mychart.ingress") -}}
...
## `alerta.persistentvolumeclaim`

`alerta.persistentvolumeclaim` can be used to easily add a
`PersistentVolumeClaim` resource to your chart that can be configured using
`.Values.persistence`:

|           Value           |                                               Description                                               |
| ------------------------- | ------------------------------------------------------------------------------------------------------- |
| persistence.enabled       | Whether or not to claim a persistent volume. If false, `alerta.volume.pvc` will use an emptyDir instead |
...
| persistence.existingClaim | If defined, `PersistentVolumeClaim` is not created and `alerta.volume.pvc` helper uses this claim       |
...
```yaml
{{- template "alerta.persistentvolumeclaim" (list . "mychart.persistentvolumeclaim") -}}
...
env:
- {{ template "alerta.envvar.value" (list "ZEUS" "cat") }}
- {{ template "alerta.envvar.secret" (list "ATHENA" "secret-name" "athena") }}
{{- end -}}
```
...
spec:
  template:
    spec:
      volumes:
      - {{ template "alerta.volume.configMap" (list "config" "configmap-name") }}
      - {{ template "alerta.volume.pvc" (list "data" "pvc-name" .Values.persistence) }}
{{- end -}}
```
...
The `alerta.volume.pvc` helper uses the following configuration from the `.Values.persistence` object:
...
The `alerta.fullname` template generates a name suitable for the `name:` field
...
```yaml
{{- define "my.fullname" -}}
  {{ template "alerta.fullname" . }}-my-stuff
{{- end -}}
```

### `alerta.fullname.unique`

The `alerta.fullname.unique` variant of fullname appends a unique seven-character
sequence to the end of the alerta name field.

This takes all of the same parameters as `alerta.fullname`

Example template:

```yaml
uniqueName: {{ template "alerta.fullname.unique" . }}
```
...
The `alerta.name` template generates a name suitable for the `app` label. It is used like this:
...
```yaml
{{- define "my.name" -}}
  {{ template "alerta.name" . }}-my-stuff
{{- end -}}
```

### `alerta.metadata`

The `alerta.metadata` helper generates the `metadata:` section of a Kubernetes
resource.
...
```yaml
{{ template "alerta.metadata" (dict "top" . "metadata" .Values.bio) }}
---
{{ template "alerta.metadata" (dict "top" . "metadata" .Values.pet "fullnameOverride" .Values.pet.fullnameOverride) }}
```
...
Most of the alerta templates that define a resource type (e.g. `alerta.configmap`
or `alerta.job`) use this to generate the metadata, which means they inherit
the same `labels`, `annotations`, `nameOverride`, and `hook` fields.

### `alerta.labelize`

`alerta.labelize` turns a map into a set of labels.

Example template:

```yaml
{{- $map := dict "first" "1" "second" "2" "third" "3" -}}
{{- template "alerta.labelize" $map -}}
```
...
### `alerta.labels.standard`

`alerta.labels.standard` prints the standard set of labels.

Example usage:

```
{{ template "alerta.labels.standard" . }}
```
...
The `alerta.hook` template is a convenience for defining hooks.

Example template:

```yaml
{{ template "alerta.hook" "pre-install,post-install" }}
```
...
The `alerta.chartref` helper prints the chart name and version, escaped to be
legal in a Kubernetes label field.

Example template:

```yaml
chartref: {{ template "alerta.chartref" . }}
```
